...
class VehicleScanScreen extends StatefulWidget {
  ...
  const VehicleScanScreen({Key? key, this.qrdata, required this.operationData})
      : super(key: key);
...
class _VehicleScanScreenState extends State<VehicleScanScreen> {
  ...
  void initState() {
    ...
    provider.loadVehicleDisplayData(widget.qrdata);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: FAppBar.getCommonAppBar(title: "Vehcile Scan"),
      body: Consumer<CommonScanProvider>(builder: (context, value, child) {
        return provider.dropDowns != null &&
                provider.commonOperationVehicle != null
            ? ListView(
                padding: EdgeInsets.all(10),
                children: [
                  CardSeperateRow(
                    "Vehicle Type",
                    provider.commonOperationVehicle!.data!.vechileType,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 5,
                  ...
                  CardSeperateRow(
                    "Vehicle Reg.No",
                    provider.commonOperationVehicle!.data!.vechileNo,
                    fontsize: 16,
                  ),
                  ...
                  CardSeperateRow(
                    "Landmark",
                    provider.commonOperationVehicle!.data!.landmark,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 5,
                  ...
                  CardSeperateRow(
                    "Area",
                    provider.commonOperationVehicle!.data!.area,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 5,
                  ...
                  CardSeperateRow(
                    "Ward",
                    provider.commonOperationVehicle!.data!.wardName,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 5,
                  ...
                  CardSeperateRow(
                    "Circle",
                    provider.commonOperationVehicle!.data!.circle,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 5,
                  ...
                  CardSeperateRow(
                    "Zone",
                    provider.commonOperationVehicle!.data!.zone,
                    fontsize: 16,
                  ),
                  SizedBox(
                    height: 10,
                    ...
                    child: GradientButton(
                      onclick: () async {
                        FormData formdata = FormData.fromMap({
                          'db_type': provider.commonOperationVehicle?.data?.dbType,

                          'user_id': Globals.userData?.data?.userId ?? "",
                          'collection_id':
                              provider.commonOperationVehicle?.data?.colId ?? "",
                          'wt_type': _weight,
                          'picked_denied':
